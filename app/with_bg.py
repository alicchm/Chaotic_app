import tkinter as tk
from tkinter import CENTER, LEFT, RIGHT, Toplevel, ttk, filedialog
from PIL import ImageTk, Image
import matplotlib.pyplot as plt
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
import numpy as np
from io import BytesIO
import win32clipboard

#na czas implementacji
from tkinter.messagebox import showinfo

def enc_openimage():
    enc_filename = filedialog.askopenfilename(
        title = 'Wybierz obraz',
        filetypes=[('Obraz JPG','*.jpg'),('Obraz PNG','*.png')]
    )

    showinfo(title='Wybrany plik', message=enc_filename)

def enc_selectalgorithm():
    print('wybrano algorytm: ', enc_option_radio.get())

def enc_open_window():
    enc2_window = Toplevel(root)
    enc2_window.geometry('566x345')
    enc2_window.title('Chaotyczny program - po szyfrowaniu')
    enc2_window.configure(bg='#242424')
    enc2_window.resizable(False, False)

    #strona z zakodowanym obrazem
    page2_encode_results = tk.Frame(enc2_window, width=566, height=325, bg="#242424")
    page2_encode_results.grid(column=0, row=0, sticky='news')

    #tło strony
    enc2_bg_img = ImageTk.PhotoImage(Image.open("app/encode2_bg2.jpg").resize((566,325), Image.Resampling.LANCZOS))
    enc2_bgimg_label = tk.Label(page2_encode_results, image=enc2_bg_img, bg='#242424')
    enc2_bgimg_label.img = enc2_bg_img  
    enc2_bgimg_label.place(relx=0.5, rely=0.5, anchor='center')
    
    #wyświetlenie zakodowanego obrazu
    enc2_img_base = Image.open("app/encoded.png")
    enc2_img = ImageTk.PhotoImage(enc2_img_base.resize((270,270), Image.Resampling.LANCZOS))
    enc2_img_label = tk.Label(page2_encode_results, image=enc2_img)
    enc2_img_label.img = enc2_img  
    enc2_img_label.place(relx=0.315, rely=0.5, anchor='center')

    #button - zapisz obraz
    enc2_save_button = tk.Button(page2_encode_results, text = 'Zapisz obraz', width=15, height=1, bg='#93A295', bd=0)
    enc2_save_button.place(relx=0.72, rely=0.14)

    #button - kopiuj obraz do schowka
    enc2_copy_button = tk.Button(page2_encode_results, text = 'Kopiuj obraz', width=15, height=1, bg='#93A295', bd=0, command=lambda:copy_to_clipboard(enc2_img_base))
    enc2_copy_button.place(relx=0.72, rely=0.31)

    #label - klucz
    enc2_key_label = tk.Label(page2_encode_results, text='Klucz:', bg='#93A295', width=15, justify=CENTER)
    enc2_key_label.place(relx=0.72, rely=0.49)

    #label - wartość klucza
    enc2_keyval_label = tk.Label(page2_encode_results, text='1233456787654', bg='#CED4CF')
    enc2_keyval_label.place(relx=0.742, rely=0.578)

    

    #strona z miarami jakości
    page2_encode_measures = tk.Frame(enc2_window, width=566, height=325, bg="#242424")
    page2_encode_measures.grid(column=0, row=0, sticky='news')
    
    #tło strony
    enc2_meas_bg_img = ImageTk.PhotoImage(Image.open("app/encode2_measures_bg.jpg").resize((566,325), Image.Resampling.LANCZOS))
    enc2_meas_bgimg_label = tk.Label(page2_encode_measures, image=enc2_meas_bg_img, bg='#242424')
    enc2_meas_bgimg_label.img = enc2_meas_bg_img  
    enc2_meas_bgimg_label.place(relx=0.5, rely=0.5, anchor='center')

    #button - do wyników kodowania
    enc2_toresults_button = tk.Button(page2_encode_measures, text = 'Powrót', width=8, height=1, bg='#CED4CF', bd=0, command=lambda: change_frame(page2_encode_results))
    enc2_toresults_button.place(relx=0.04, rely=0.885)

    fig = plt.figure(figsize=(3,2),dpi=100)
    ax = fig.add_axes([0,0.1,0.8,0.8])
    line = FigureCanvasTkAgg(fig, page2_encode_measures)
    line.get_tk_widget().pack(side=tk.LEFT)
    line.draw()

    #button - do strony z miarami jakości
    enc2_tomeasures_button = tk.Button(page2_encode_results, text = 'Miary', width=8, height=1, bg='#CED4CF', bd=0, command=lambda: change_frame_calc(page2_encode_measures, line, ax))
    enc2_tomeasures_button.place(relx=0.86, rely=0.885)

    change_frame(page2_encode_results) #żeby na starcie była otwarta pierwsza strona

def change_frame(frame):
    frame.tkraise()

def change_frame_calc(frame, line, ax):
    change_frame(frame)

    #obliczenie miar itp

    #przykładowy wykres
    ax.clear()
    t = np.arange(0.0, 2.0, 0.01)
    s = 1 + np.sin(2 * np.pi * t)
    # fig = plt.Figure(figsize=(3,2), dpi=100)
    # ax = fig.add_subplot(111)
    ax.plot(t,s)
    line.draw()
    # line.get_tk_widget().pack(side=tk.LEFT) #, fill=tk.BOTH
    # return fig

def copy_to_clipboard(image1): #kopiowanie obrazu do schowka
    output = BytesIO()
    image1.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    output.close()

    win32clipboard.OpenClipboard()
    win32clipboard.EmptyClipboard()
    win32clipboard.SetClipboardData(win32clipboard.CF_DIB, data)
    win32clipboard.CloseClipboard()

def dec_open_window():
    dec2_window = Toplevel(root)
    dec2_window.geometry('566x345')
    dec2_window.title('Chaotyczny program - po deszyfrowaniu')
    dec2_window.configure(bg='#242424')
    dec2_window.resizable(False, False)

    #wyświetlenie odkodowanego obrazu
    dec2_img_base = Image.open("app/lena.jpg")
    dec2_img = ImageTk.PhotoImage(dec2_img_base.resize((270,270), Image.Resampling.LANCZOS))
    dec2_img_label = tk.Label(dec2_window, image=dec2_img)
    dec2_img_label.img = dec2_img  
    dec2_img_label.place(relx=0.315, rely=0.5, anchor='center')

    #button - zapisz obraz
    dec2_save_button = tk.Button(dec2_window, text = 'Zapisz obraz', width=15, height=1, bg='#93A295', bd=0)
    dec2_save_button.place(relx=0.72, rely=0.14)

    #button - kopiuj obraz do schowka
    dec2_copy_button = tk.Button(dec2_window, text = 'Kopiuj obraz', width=15, height=1, bg='#93A295', bd=0, command=lambda:copy_to_clipboard(dec2_img_base))
    dec2_copy_button.place(relx=0.72, rely=0.31)

#główne okno
root = tk.Tk()
root.geometry('566x345')
root.title('Chaotyczny program')
root.configure(bg="#242424")
root.resizable(False, False) 

root_icon = ImageTk.PhotoImage(file = 'app/app_logo_mini2.png')
root.iconphoto(False, root_icon)

#notebook = tworzenie zakładek
enc_style_notebook = ttk.Style()
enc_style_notebook.theme_use('clam')
enc_style_notebook.configure('TNotebook.Tab', background='#dadad9')
enc_style_notebook.configure('TNotebook', background='#242424', bordercolor='#242424')
enc_style_notebook.map("TNotebook.Tab", background= [('selected', '#242424')], foreground=[('selected','#ffffff')])

notebook = ttk.Notebook(root, style='TNotebook')
notebook.grid(column=0, row=0)

#tworzenie zakładek i dodanie ich do notebooka
page_encode = tk.Frame(notebook, width=566, height=325, bg="#242424")
page_decode = tk.Frame(notebook, width=566, height=325, bg="#242424")

page_encode.grid(sticky='NESW')
page_decode.grid(sticky='NESW')

notebook.add(page_encode, text='Szyfrowanie')
notebook.add(page_decode, text='Deszyfrowanie')

#strona z kodowaniem
#tło strony
enc_bg_img = ImageTk.PhotoImage(Image.open("app/encode_bg5.jpg").resize((566,325), Image.Resampling.LANCZOS))
enc_bgimg_label = tk.Label(page_encode, image=enc_bg_img, bg='#242424')
enc_bgimg_label.img = enc_bg_img  
enc_bgimg_label.place(relx=0.5, rely=0.5, anchor='center')

#przycisk ładowania obrazu
#page_encode.wm_attributes('-transparentcolor')
enc_loadimg_button = tk.Button(page_encode, text = 'Ładuj obraz', command=enc_openimage, width=15, height=1, bg='#e59500', bd=0)
enc_loadimg_button.place(relx=0.056, rely=0.1)

#wybór algorytmu
enc_option_radio = tk.IntVar()
enc_style_radiobutton = ttk.Style()
enc_style_radiobutton.configure('enc_radio.TRadiobutton', background='#e59500')

enc_algorithm1_radio = ttk.Radiobutton(page_encode, text='Algorytm 1.', variable=enc_option_radio, value=1, command=enc_selectalgorithm, style='enc_radio.TRadiobutton')
enc_algorithm1_radio.place(relx=0.07, rely=0.29)

enc_algorithm2_radio = ttk.Radiobutton(page_encode, text='Algorytm 2.', variable=enc_option_radio, value=2, command=enc_selectalgorithm, style='enc_radio.TRadiobutton')
enc_algorithm2_radio.place(relx=0.07, rely=0.38)

#wartości x i p
enc_x_label = tk.Label(page_encode, text='x:', bg='#e59500')
enc_x_label.place(relx=0.07, rely=0.57)

enc_x_spinbox = tk.Spinbox(page_encode, from_=0, to_=1, format='%10.4f', increment=0.001, validate='focusout', width=10, relief='flat', 
                    bd=0, buttondownrelief=tk.FLAT, buttonuprelief=tk.FLAT, bg='#FFC969')
enc_x_spinbox.place(relx=0.105, rely=0.576)

enc_p_label = tk.Label(page_encode, text='p:', bg='#e59500')
enc_p_label.place(relx=0.07, rely=0.66)

enc_p_spinbox = tk.Spinbox(page_encode, from_=0, to_=1, format='%10.4f', increment=0.001, validate='focusout', width=10, relief='flat', 
                    bd=0, buttondownrelief=tk.FLAT, buttonuprelief=tk.FLAT, bg='#FFC969')
enc_p_spinbox.place(relx=0.105, rely=0.666)

#szyfrowanie - start algorytmu
enc_encode_button = tk.Button(page_encode, text = 'Szyfruj!', width=15, height=1, bg='#e59500', bd=0, command=enc_open_window)
enc_encode_button.place(relx=0.056, rely=0.82)

#wyświetlanie załadowanego obrazu
enc_image = Image.open("app/lena.jpg")
enc_image = enc_image.resize((270,270), Image.Resampling.LANCZOS)
enc_image_tk = ImageTk.PhotoImage(enc_image)

enc_image_label = ttk.Label(page_encode, image=enc_image_tk)
enc_image_label.place(relx=0.425, rely=0.082)

#strona z dekodowaniem
#tło strony
dec_bg_img = ImageTk.PhotoImage(Image.open("app/encode_bg5.jpg").resize((566,325), Image.Resampling.LANCZOS))
dec_bgimg_label = tk.Label(page_decode, bg='#242424') #image=dec_bg_img,
# dec_bgimg_label.img = enc_bg_img  
# dec_bgimg_label.place(relx=0.5, rely=0.5, anchor='center')

#przycisk ładowania obrazu
#page_encode.wm_attributes('-transparentcolor')
dec_loadimg_button = tk.Button(page_decode, text = 'Ładuj obraz', command=enc_openimage, width=15, height=1, bg='#e59500', bd=0)
dec_loadimg_button.place(relx=0.056, rely=0.1)

#wybór algorytmu
dec_option_radio = tk.IntVar()
dec_style_radiobutton = ttk.Style()
dec_style_radiobutton.configure('dec_radio.TRadiobutton', background='#e59500')

dec_algorithm1_radio = ttk.Radiobutton(page_decode, text='Algorytm 1.', variable=dec_option_radio, value=1, command=enc_selectalgorithm, style='dec_radio.TRadiobutton')
dec_algorithm1_radio.place(relx=0.07, rely=0.29)

dec_algorithm2_radio = ttk.Radiobutton(page_decode, text='Algorytm 2.', variable=dec_option_radio, value=2, command=enc_selectalgorithm, style='dec_radio.TRadiobutton')
dec_algorithm2_radio.place(relx=0.07, rely=0.38)

#wartości x i p
dec_key_label = tk.Label(page_decode, text='klucz:', bg='#e59500')
dec_key_label.place(relx=0.07, rely=0.57)

dec_key_spinbox = tk.Spinbox(page_decode, from_=0, to_=100, format='%10.0f', increment=1, validate='focusout', width=10, relief='flat', 
                    bd=0, buttondownrelief=tk.FLAT, buttonuprelief=tk.FLAT, bg='#FFC969', justify=tk.RIGHT)
dec_key_spinbox.place(relx=0.105, rely=0.676)

#szyfrowanie - start algorytmu
dec_encode_button = tk.Button(page_decode, text = 'Deszyfruj!', width=15, height=1, bg='#e59500', bd=0, command=dec_open_window)
dec_encode_button.place(relx=0.056, rely=0.82)

#wyświetlanie załadowanego obrazu
dec_image = Image.open("app/encoded.png")
dec_image = dec_image.resize((270,270), Image.Resampling.LANCZOS)
dec_image_tk = ImageTk.PhotoImage(dec_image)

dec_image_label = ttk.Label(page_decode, image=dec_image_tk)
dec_image_label.place(relx=0.425, rely=0.082)

root.mainloop()